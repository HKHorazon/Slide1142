...
1. `to procedure do` (做一件事情)
2. `to procedure result` (計算一個結果)

我們先從最簡單的 **Do (做一件事情)** 開始！

---

# 建立你的第一個程序

1. 拉出 `to procedure do` 積木。
2. 點擊 `procedure` 這個字，幫它改個好懂的名字。
   - 例如：`ResetGame` (重置遊戲)。
3. 把你要執行的動作（分數歸零、敵人回原點）塞進 `do` 的肚子裡。
...
定義：
`to UpdateScore (points) do:`
`設 全域分數 = 全域分數 + points`
`設 標籤.文字 = 全域分數`

呼叫：
- 打中小兵：`call UpdateScore points [10]`
- 被陷阱扣血：`call UpdateScore points [-5]`
...
呼叫時：
`設 標籤.文字 = call CalculateBMI height[1.7] weight[60]`
...
AI2 內建的許多積木，其實就是幫你寫好的程序：

- `call Notifier.ShowAlert`：是一個 **Do 程序**。
- `Math.random integer`：是一個 **Result 程序**。

今天，你學會了自己創造積木。
這是從「新手」邁向「進階開發者」最重要的一步！
...
3. **Do vs Result**：執行一串動作 vs 進行運算並回傳數值。
